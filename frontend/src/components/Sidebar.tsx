  ...
  return (
    <div className="hidden lg:flex w-[15%] flex-col bg-sidebar border-r border-sidebar-border h-[calc(100vh-65px)] sticky top-[65px]">
      ...
      <nav className="p-3 space-y-1">
        {navItems.map((item) => (
          <button key={item.label} className="flex items-center gap-3 w-full px-3 py-2.5 rounded-lg text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-sidebar-accent transition-colors">
      ...
      <div className="mt-2 px-3 flex-1 overflow-hidden flex flex-col pb-4">
        ...
        <div className="flex-1 overflow-y-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden space-y-0.5">
          {playlists.map((playlist) => (
            <button key={playlist} className="w-full text-left px-3 py-2 rounded-md text-sm text-muted-foreground hover:text-foreground hover:bg-sidebar-accent transition-colors truncate">
              {playlist}
            </button>
          ))}
        </div>
